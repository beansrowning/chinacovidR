...
pull_nhc_article_links <- function(root_url = "http://en.nhc.gov.cn", previous_run = NULL) {
  ...
  first_page_url <- paste(root_url, "DailyBriefing.html", sep = "/")
  ...
  if (!is.null(previous_run)) {
    ...
    # from a previous run
    to_bind <- article_df[!date %in% prev_run_data[["date"]], ]
  ...
  repeat {
    next_page_url <- sprintf("%s/DailyBriefing_%d.html", root_url, pg_num)
    ...
    if (!is.null(previous_run)) {
      ...
      # from a previous run
      to_bind <- page_df[!date %in% article_df[["date"]], ]
